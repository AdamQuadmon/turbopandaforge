...
const getPreferredValue = (
  ...
  const minScreenSizeMaxSize = minScreenSize * maxSize
  const maxScreenSizeMinSize = maxScreenSize * minSize

  const remCalc = cleanNumber(
    (minScreenSizeMaxSize - maxScreenSizeMinSize) / (minScreenSize - maxScreenSize),
  )
